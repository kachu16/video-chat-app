...
var myPeer = new Peer();
let count = 0;
let myVideoStream; //the video stream stored
const myVideo = document.createElement('video'); // contains the video
let currentPeer = null;
myVideo.muted = true;
const peers = {};
const names = {};
const userNamee = prompt("Please enter your name", "");
...
navigator.mediaDevices.getUserMedia({
    video: true, //want video
    audio: true //want audio
}).then(stream => {
    myVideoStream = stream; //
    addVideoStream(myVideo, stream, userNamee); 
    ...
    myPeer.on('call', call => {
        ...
        call.on('stream', userVideoStream => {
            ...
            addVideoStream(video, userVideoStream, userNamee)
        })
        currentPeer = call;
    })
    socket.on('user-connected', (userId) => {
        setTimeout(connectToNewUser, 1000, userId, userNamee, stream);
    });
...
myPeer.on('open', id => {
    socket.emit('join-room', ROOM_ID, id, userNamee);
})
